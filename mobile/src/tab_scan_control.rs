    ...
    pub fn ui(&mut self, ui: &mut egui::Ui, hybridanalysis_tag_ignorelist: &str) {
        ...
        for (idx, package) in installed_packages.iter().enumerate() {
            ...
            interactive_table = interactive_table.row(|table_row| {
                ...
                let (has_app_info, app_icon, display_title, display_subtitle) =
                    if let (Some(title), Some(developer)) = (app_title.as_ref(), app_developer.as_ref()) {
                        (true, app_texture_id, title.clone(), developer.clone())
                    } else {
                                    ...
                                    egui::ScrollArea::horizontal()
                                        ...
                                        .show(ui, |ui| {
                                            if has_app_info {
                                                ...
                                                ui.add(egui::Label::new(&display_title).wrap());
                                            }
                                        });
                                    // Subtitle (developer or empty)
                                    if !display_subtitle.is_empty() {
                                        ui.label(
                                            egui::RichText::new(&display_subtitle)
                                                .small()
